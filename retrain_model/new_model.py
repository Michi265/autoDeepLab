...
class newModel(nn.Module):
    def __init__(self, network_arch, cell_arch, num_classes, num_layers, filter_multiplier=20, lock_multiplier=5, step=5, cell=Cell,
        ...
        for i in range(self._num_layers):
            level_option = torch.sum(self.network_arch[i])
            prev_level_option = torch.sum(self.network_arch[i - 1])
            prev_prev_level_option = torch.sum(self.network_arch[i - 2])
            ...
            if i == 0:
                ...
                three_branch_options = torch.sum(self.network_arch[i])
